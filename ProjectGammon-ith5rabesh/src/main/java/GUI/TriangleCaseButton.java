...
public class TriangleCaseButton extends CaseButton{
	...
	private boolean hasQuestionStation; // Track if this triangle has a question station



	public void setHasQuestionStation(boolean hasQuestionStation) {
	    this.hasQuestionStation = hasQuestionStation;
	}

	// Update paintComponent to handle question station rendering
	@Override
	...
	public boolean hasSurpriseStation() {
	    for (int i = 0; i < getComponentCount(); i++) {
	        if (getComponent(i) instanceof SurpriseStationBarr) {
	            SurpriseStationBarr surpriseStation = (SurpriseStationBarr) getComponent(i);
	            return !surpriseStation.isUsed(); // Check if the station is unused
	        }
	    }
	    return false;
	}

	public void useSurpriseStation() {
	    for (int i = 0; i < getComponentCount(); i++) {
	        if (getComponent(i) instanceof SurpriseStationBarr) {
	            SurpriseStationBarr surpriseStation = (SurpriseStationBarr) getComponent(i);
	            if (!surpriseStation.isUsed()) {
	                surpriseStation.markAsUsed(); // Mark the station as used and remove `$`
	            }
	        }
	    }
	}
